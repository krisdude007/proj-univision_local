...
if (sizeof($videos) != 0) {
    foreach ($videos as $video) {
        ...
        echo sprintf(
                $videoFormat, '/play/' . $video->view_key, '/' . basename(Yii::app()->params['paths']['video']) . "/{$video->thumbnail}.png",  $videoThumbnailWidth, $video->title, date("F j, Y", strtotime($video->created_on)) . " " . date("T"), $userLink, $byName, $video->views, $stars
        );
    }
}
?>
